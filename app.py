...
# 1. Asset Selection (Real International Forex Pairs)
...
if st.button("RUN MATHEMATICAL ANALYSIS 🚀"):
    with st.spinner("Fetching live tick-by-tick mathematical data matrices..."):
        ...
        if data.empty:
            ...
            # Extract Close prices safely as a clean 1D Series
            ...
            if len(close_prices) < 50:
                st.error("⚠️ Not enough data points to compute math matrices.")
            else:
                st.success(f"✅ Connection Established. Analyzing last {len(close_prices)} price vectors.")
                ...
                # Safely get the very last scalar value
                ...
                current_macd = float(macd_line.values[-1])
                current_signal = float(signal_line.values[-1])
                
                real_support = float(low_prices.iloc[-50:].min())
                real_resistance = float(high_prices.iloc[-50:].max())
